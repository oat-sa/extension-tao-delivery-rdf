...
;filterFactory($filterContainer,{placeholder:__("Select the test you want to publish to the test-takers"),width:"64%",quietMillis:1e3,label:__("Select the test")}).on("change",function(test){$formElement.val(test),test?taskCreationButton.enable():taskCreationButton.disable()}).on("request",function(params){provider.list(params.data).then(function(tests){params.success(tests)}).catch(function(err){params.error(err),feedback().error(err)})}).render("<%= text %>"),$oldSubmitter=$form.find(".form-submitter"),taskCreationButton=taskCreationButtonFactory({type:"info",icon:"delivery",title:__("Publish the test"),label:__("Publish"),taskQueue:taskQueue,
taskCreationUrl:$form.prop("action"),taskCreationData:function(){return $form.serializeArray()},taskReportContainer:$container}).on("finished",function(result){result.task&&result.task.report&&_.isArray(result.task.report.children)&&result.task.report.children.length&&result.task.report.children[0]&&(result.task.report.children[0].data&&result.task.report.children[0].data.uriResource?(feedback().info(__("%s completed",result.task.taskLabel)),refreshTree(result.task.report.children[0].data.uriResource)):this.displayReport(result.task.report.children[0],__("Error")))}).on("continue",function(){refreshTree()}).on("error",function(err){feedback().error(err)
}).render($oldSubmitter.closest(".form-toolbar")).disable(),$oldSubmitter.replaceWith(taskCreationButton.getElement().css({float:"right"}))}}}),define("taoDeliveryRdf/controller/Publish/index",["jquery","lodash","i18n","util/url","layout/actions","provider/resources","ui/destination/selector","ui/feedback","core/taskQueue/taskQueue"],function($,_,__,urlHelper,actionManager,resourceProviderFactory,destinationSelectorFactory,feedback,taskQueue){"use strict";var refreshTree=function(uriResource){actionManager.trigger("refresh",{uri:uriResource})};return{start:function(){
var $container=$(".selector-container"),testUri=$container.data("test"),testLabel=$container.data("label"),resourceProvider=resourceProviderFactory();destinationSelectorFactory($container,{title:__('Publish "%s" to',testLabel),actionName:__("Publish"),icon:"delivery",taskQueue:taskQueue,taskCreationData:{testUri:testUri},taskCreationUrl:urlHelper.route("publish","Publish","taoDeliveryRdf"),classUri:$container.data("root-class")}).on("query",function(params){var self=this;params.classOnly=!0,resourceProvider.getResources(params,!0).then(function(resources){self.update(resources,params)}).catch(function(err){self.trigger("error",err)})
}).on("finished",function(result,button){result.task&&result.task.report&&_.isArray(result.task.report.children)&&result.task.report.children.length&&result.task.report.children[0]&&(result.task.report.children[0].data&&result.task.report.children[0].data.uriResource?(feedback().info(__("%s completed",result.task.taskLabel)),refreshTree(result.task.report.children[0].data.uriResource)):button.displayReport(result.task.report.children[0],__("Error")))}).on("continue",function(){refreshTree(testUri)}).on("error",function(err){feedback().error(err)})}}});
